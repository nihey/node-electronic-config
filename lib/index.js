...
function perShell(electronicConfiguration) {
  ...
  subLevels.forEach(function(config) {
    ...
    var split = config.split(/[spdf]/).map(function(n) {return parseInt(n)});
    var level = split[0];
    var sum = split[1];
 ...
 */
module.exports = function(electronicConfiguration) {
  var shells = perShell(electronicConfiguration);
  return shells.filter(function(electrons) {return electrons});
};
